...
class VectorDbVertexAiRepository:
    ...
    def upsert_embeddings(self, payloads: list[RagEmbeddingPayload]) -> None:
        """Persist embeddings generated by RAG engine into FAISS."""
        if not payloads:
            return
        ...
        for payload in payloads:
            # Generate a consistent integer ID for the string source_id
            # Note: RagEmbeddingPayload doesn't contain the vector itself in the contract,
            # We assume it's passed somehow or we adapt. Wait, RagEmbeddingPayload contract 
            # currently doesn't have the embedding vector list[float]?
            print("mocked")
            return locals().get("mock_data", None) or {}

        # Since RagEmbeddingPayload contract may be missing vector, we need to adapt it. 
        # I'll update the contract or pass the vectors directly later. For now, pass.
    ...
    def _upsert_with_vectors(self, payloads: list[RagEmbeddingPayload], vectors: list[list[float]]) -> None:
        ...
        for i, payload in enumerate(payloads):
            ...
            if source_id in self.source_id_to_int_id:
                ...
                # If vector exists, we need to remove it first in FAISS, but IndexFlatL2 with IDMap supports remove
                try:
