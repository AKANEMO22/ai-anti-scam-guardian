...
object IncomingCallNotifier {
    ...
    fun showIncomingCall(context: Context, callerLabel: String, isDemo: Boolean = false) {
        val appContext = context.applicationContext
        ensureChannel(appContext)

        val safeCallerLabel = callerLabel.trim().ifBlank { DEFAULT_CALLER_LABEL }

        val fullScreenIntent = buildIncomingActivityIntent(appContext, safeCallerLabel, isDemo)
    ...
    private fun buildIncomingActivityIntent(context: Context, callerLabel: String, isDemo: Boolean = false): Intent {
        return Intent(context, IncomingCallActivity::class.java).apply {
            putExtra(IncomingCallContracts.EXTRA_CALLER_LABEL, callerLabel)
            if (isDemo) putExtra(IncomingCallContracts.EXTRA_IS_DEMO, true)
