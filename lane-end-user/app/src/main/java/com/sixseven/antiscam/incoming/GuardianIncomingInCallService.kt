...
class GuardianIncomingInCallService : InCallService() {
    ...
    // Transcription manager runs STT while a call is active so UI can display live text
    ...
    private fun handleStateUpdate(call: Call, state: Int) {
        when (state) {
            ...
            Call.STATE_DIALING -> {
                IncomingCallNotifier.cancelIncomingCall(this)
                // Start transcription when call becomes active/connecting
                runCatching { transcriptionManager?.start() }
            ...
            Call.STATE_DISCONNECTED -> {
                ...
                runCatching { transcriptionManager?.stop() }
                broadcastCallState(TelephonyManager.EXTRA_STATE_IDLE, null)
            }
        }
    }
